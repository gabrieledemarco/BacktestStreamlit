...
def calculate_metrics(results):
    ...
    # Safeguard for computing Sharpe Ratio
    ...
    if excess_returns.std() != 0:
        ...
        sharpe_ratio = 0.0  # Assign a default value if the standard deviation is zero

    # Maximum drawdown
    rolling_max = results['portfolio_value'].cummax()
    drawdowns = (results['portfolio_value'] - rolling_max) / rolling_max
    max_drawdown = drawdowns.min() * 100 if rolling_max.max() > 0 else 0.0
    ...
    if len(trades) > 0:
        ...
        win_rate = 0.0  # If there are no trades, win rate is zero
